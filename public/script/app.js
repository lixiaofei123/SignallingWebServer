...
var _showConnectOverlayTimeout;
...
function showPlayOverlay() {
	...
	clearTimeout(_showPlayOverlayTimeout);
	_showPlayOverlayTimeout = setTimeout(function () {
		// if (webRtcPlayerObj){
		// webRtcPlayerObj.video.play();
		// console.log(`showPlayOverlay play`);
		...
		connectSuccess = true
		for (let listener of connectSuccessListeners) {
			listener()
		}
...
window.sendTestMessage = message => {
...
function emitDescriptor(messageType, descriptor) {
	...
	let descriptorAsString = "";
	if (typeof descriptor === "string") {
		descriptorAsString = descriptor
	} else {
...
function emitUIInteraction(descriptor) {
	let messageStr = JSON.stringify(descriptor)
	console.log("向像素流发送消息--->", messageStr)
...
function setMessageListener() {
	window.addEventListener("message", (event) => {
		if (typeof event.data === "string") {
			...
			if (data.type === "emitCommand") {
				emitCommand(data.desc)
			} else if (data.type === "emitUIInteraction") {
				emitUIInteraction(data.desc)
			}
		}
...
var url = window.config.signServerUrl;

function load() {

	Ajax.get(`${window.config.signServerApiUrl}/status`, respText => {
		let status = JSON.parse(respText)
		if (status.freenum === 0) {
			if (!window.config.debug) {
				let errorText = `没有可用的像素流实例\n已经启动${status.count}个实例，已开启实例被下面IP占用`
				for (let ip of status.ips) {
					errorText = errorText + "\n" + ip
				}
			...
			start();
			setMessageListener();
			addResponseEventListener("printMessage", data => {
				data = JSON.stringify(data)
				console.log("收到像素流消息--->", data)
			})
		}

	}, () => {
		handleError('连接服务器异常，请联系管理员 \n')
	})
